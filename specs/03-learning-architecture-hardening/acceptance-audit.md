...
| 9 | Zero behavior regression proven by real Obsidian smoke: new course -> intake -> plan chapter -> write note -> continue section -> Start new lesson -> restart restore. | Pending | `npm run learning:smoke-ready -- --fresh` passed and prepared `ai-tutor-smoke-vault` with the plugin enabled and no existing courses. `npm run learning:open-smoke-vault -- --print` prints the Obsidian URI. The real Obsidian smoke has not been confirmed yet. |

## Remaining Gate

1. Run `npm run learning:smoke-ready -- --fresh`.
2. Run `npm run learning:open-smoke-vault`, or open `D:\claudian\ai-tutor-smoke-vault` in Obsidian.
