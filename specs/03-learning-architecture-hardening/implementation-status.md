...
- `npm run learning:open-smoke-vault` opens the prepared clean vault via Obsidian URI; `-- --print` prints the URI without opening the app.
...
- `npm run learning:open-smoke-vault -- --print`: passed.
...
- Before smoke, run `npm run learning:smoke-ready -- --fresh`, then `npm run learning:open-smoke-vault`; use `manual-smoke-checklist.md` in this spec folder to record the user-side smoke result.
