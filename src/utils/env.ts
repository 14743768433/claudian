...
function getAppProvidedCliPaths(): string[] {
  if (process.platform === 'darwin') {
    ...
    return [path.posix.dirname(process.execPath)];
  }
...
export function findNodeDirectory(additionalPaths?: string): string | null {
  ...
  for (const dir of allPaths) {
    if (!dir) continue;
    try {
      const nodePath = joinPath(dir, NODE_EXECUTABLE);
...
export function findNodeExecutable(additionalPaths?: string): string | null {
  ...
  if (nodeDir) {
    return joinPath(nodeDir, NODE_EXECUTABLE);
  }
  return null;
}
...
export function getEnhancedPath(additionalPaths?: string, cliPath?: string): string {
  ...
  if (cliPath) {
    try {
      const cliDir = dirnamePath(cliPath);
      const nodeInCliDir = joinPath(cliDir, NODE_EXECUTABLE);
