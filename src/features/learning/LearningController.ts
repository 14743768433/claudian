...
import { CourseArtifactsView } from './views/CourseArtifactsView';
...
function isChapterListView(value: unknown): value is ChapterListView {
  return value instanceof ChapterListView;
}

function isArtifactsView(value: unknown): value is CourseArtifactsView {
  return value instanceof CourseArtifactsView;
}

function isLibraryView(value: unknown): value is CourseLibraryView {
  return value instanceof CourseLibraryView;
}
...
export class LearningController {
  ...
  async initialize(): Promise<void> {
    ...
    await this.skillSeeder.seedVaultSkills().catch(() => {
      new Notice('AI Tutor could not seed .claude skill templates.');
    });
  }
  ...
  async createCourse(input: { title: string; goalTitle: string }): Promise<CourseState> {
    ...
    await this.plugin.renameConversation(conversation.id, `${input.title} · Intake`);
    ...
    new Notice('Course created. Read or add source material, then ask AI Tutor to plan the course.');
  ...
  async enterCourse(courseId: string): Promise<void> {
    ...
    if (!course) {
      new Notice('AI Tutor course state could not be loaded.');
      return;
    }
    this.cacheCourse(course);
  ...
  async enterLesson(courseId: string, lessonId: string): Promise<void> {
    ...
    if (!lesson) {
      new Notice('AI Tutor lesson could not be found.');
      return;
    }
  ...
  async advanceSectionFromConversation(conversationId: string): Promise<void> {
    const ref = await this.stateService.findByConversationId(conversationId);
    if (!ref) {
      new Notice('AI Tutor could not find a course for this conversation.');
    ...
    if (!result.ok || !result.state) {
      new Notice(result.message ?? 'AI Tutor could not continue to the next section.');
      this.refreshOpenChatLearningControls();
      return;
    }
  ...
  async writeSectionNoteFromConversation(conversationId: string): Promise<void> {
    ...
    if (!this.isWriteSectionNoteReady(ref)) {
      new Notice('AI Tutor can only write a note for the current pending section.');
      this.refreshOpenChatLearningControls();
      return;
    }
    ...
    if (!tab || tab.state.isStreaming) {
      new Notice('AI Tutor is already working. Try writing the note after the current turn finishes.');
      this.refreshOpenChatLearningControls();
      return;
    }
  ...
  async practiceSectionFromConversation(conversationId: string): Promise<void> {
    ...
    if (!this.isPracticeSectionReady(ref)) {
      new Notice('AI Tutor can only practice the current active section.');
      this.refreshOpenChatLearningControls();
      return;
    }
    ...
    const tab = await this.findOpenTabForConversation(conversationId);
    if (!tab || tab.state.isStreaming) {
      new Notice('AI Tutor is already working. Try the practice check after the current turn finishes.');
      this.refreshOpenChatLearningControls();
      return;
    }
  ...
  async reviewLessonFromConversation(conversationId: string): Promise<void> {
    ...
    if (!this.isReviewLessonReady(ref)) {
      new Notice('AI Tutor can only review a chapter after it is finished.');
      this.refreshOpenChatLearningControls();
      return;
    }

    if (!await this.sendLessonReviewTurn(conversationId, ref.course, ref.lesson)) {
      new Notice('AI Tutor is already working. Try the chapter review after the current turn finishes.');
      this.refreshOpenChatLearningControls();
      return;
    }
  }
  ...
  async startNewLessonFromConversation(
    ...
    if (!options.force && !this.isStartNewLessonReady(ref)) {
      new Notice('Finish and cover every section before starting a new lesson.');
      this.refreshOpenChatLearningControls();
      return;
    }
    ...
    if (!result.ok || !result.state) {
      new Notice(result.message ?? 'AI Tutor could not start a new lesson.');
      this.refreshOpenChatLearningControls();
      return;
    }
  ...
  async arrangeLearningLayout(courseId: string): Promise<void> {
    ...
    if (leftLeaf) {
      ...
      if (isChapterListView(leftLeaf.view)) {
        leftLeaf.view.setCourseId(courseId);
      }
    }
    ...
    if (rightLeaf) {
      ...
      if (isArtifactsView(rightLeaf.view)) {
        rightLeaf.view.setCourseId(courseId);
      }
    }
  ...
  async handleAssistantTurnComplete(
    ...
    if (!schemaCheck) {
      new Notice('AI Tutor course-state.json is invalid or missing.');
    ...
    for (const request of requests) {
      ...
      if (actionNeedsQualityGate(request.action)) {
        ...
        if (!gate.pass) {
          new Notice(`AI Tutor note quality gate failed: ${gate.reasons.join(' ')}`);
          actionOutcomes.push({
            ...actionSummary,
            status: 'rejected',
      ...
      if (!result.ok) {
        new Notice(result.message ?? 'AI Tutor action was rejected.');
        actionOutcomes.push({
          ...actionSummary,
          status: 'rejected',
  ...
  async openNote(path: string): Promise<void> {
    ...
    if (!(file instanceof TFile)) {
      new Notice('AI Tutor note file is missing.');
      return;
    }
  ...
  async openSource(source: string | LearningLessonPlanSource): Promise<void> {
    ...
    const file = this.resolveSourceFile(requestedPath);
    if (file) {
      const leaf = this.plugin.app.workspace.getLeaf('split', 'vertical');
      await leaf.openFile(file);
      return;
    ...
    if (cardId && !requestedPath) {
      new Notice('This source is an external card, not a vault note yet.');
      return;
    }

    new Notice(`AI Tutor source could not be opened: ${label || requestedPath || 'unknown source'}.`);
  }
  ...
  private buildRepairPrompt(conversationId: string, notePath: string, reasons: string[]): string | null {
    ...
    if (nextAttempt > this.maxRepairAttempts) {
      new Notice('AI Tutor kept the note file but stopped auto-repair after repeated quality failures.');
      return null;
    }
  ...
  private async loadCurrentSectionSourceSnippets(lesson: LessonSession): Promise<SourceSnippet[]> {
    ...
    for (const source of sources) {
      ...
      seenPaths.add(path);
      const file = this.resolveSourceFile(path);
      if (!file) continue;

      const text = await this.readSourceFileSnippet(file);
      if (!text) continue;
      snippets.push({
  ...
  private async readSourceFileSnippet(file: TFile): Promise<string> {
    try {
      const vault = this.plugin.app.vault as typeof this.plugin.app.vault & {
        cachedRead?: (file: TFile) => Promise<string>;
      };
      const markdown = vault.cachedRead
        ? await vault.cachedRead(file)
        : await this.adapter.read(file.path);
      return this.compactSourceSnippet(markdown);
    } catch {
      return '';
    }
  }
  ...
  private resolveSourceFile(path: string): TFile | null {
    ...
    for (const candidate of candidates) {
      const file = metadataCache?.getFirstLinkpathDest?.(candidate.replace(/\.md$/i, ''), '');
      if (file instanceof TFile) {
        return file;
      }
    }
  ...
  private async ensureLessonConversation(course: CourseState, lesson: LessonSession): Promise<void> {
    if (this.plugin.getConversationSync(lesson.conversationId)) {
      return;
    }
    const replacement = await this.plugin.createConversation();
    await this.plugin.renameConversation(replacement.id, `${course.title} · ${lesson.title}`);
    lesson.conversationId = replacement.id;
    await this.stateService.saveCourse(course);
    new Notice('AI Tutor recreated a missing chapter conversation.');
  }
  ...
  private async openChatConversation(conversationId: string): Promise<void> {
    ...
    const view = leaf.view && typeof (leaf.view as { getTabManager?: unknown }).getTabManager === 'function'
      ? leaf.view as unknown as { getTabManager: () => { openConversation: (id: string) => Promise<void> } | null }
      : this.plugin.getView();
  ...
  private async maybeKickoffCurrentLesson(
    ...
    if (!tab) {
      new Notice('AI Tutor opened the chapter but could not find the chat tab to start it. Open the course again to retry.');
      return false;
    }
    if (tab.state.isStreaming || messagesHaveAssistantResponse(tab.state.messages)) {
      return false;
    }
  ...
  private async sendLessonReviewTurn(
    ...
    const tab = await this.findOpenTabForConversation(conversationId);
    if (!tab || tab.state.isStreaming || !tab.controllers.inputController) {
      return false;
    }
  ...
  private async refreshOpenLearningViews(courseId: string): Promise<void> {
    for (const leaf of this.plugin.app.workspace.getLeavesOfType(VIEW_TYPE_CHAPTER_LIST)) {
      if (isChapterListView(leaf.view)) {
        leaf.view.setCourseId(courseId);
      }
    }
    for (const leaf of this.plugin.app.workspace.getLeavesOfType(VIEW_TYPE_COURSE_ARTIFACTS)) {
      if (isArtifactsView(leaf.view)) {
        leaf.view.setCourseId(courseId);
      }
    }
  }
  ...
  private async hasConversationAssistantResponse(conversationId: string): Promise<boolean> {
    ...
    const syncConversation = plugin.getConversationSync?.(conversationId);
